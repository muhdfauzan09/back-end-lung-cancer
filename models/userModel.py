...
class user_role_model(db.Model):
    ...
    user_detail = db.relationship(
        'user_detail_model', backref='user_detail', lazy='select')
    admin_detail = db.relationship(
        admin_Model, backref='admin_detail', lazy='select')
...
class user_detail_model(db.Model):
    ...
    role_id = db.Column(db.Integer, db.ForeignKey(
        'user_role.role_id'), nullable=False)
