...
class patient_detail_model(db.Model):
    ...
    department_id = db.Column(db.Integer, db.ForeignKey('department_detail.department_id'), nullable=False)
    ...
    feature_detail = db.relationship("feature_detail_model", backref='feature', lazy='select')
...
class feature_detail_model(db.Model):
    ...
    patient_id = db.Column(db.Integer, db.ForeignKey('patient_detail.patient_id'), nullable=False)
